...
export const ActionDelete = ({ db }) => {
  const [zip, setZip] = useState();

  const onClick = () => {
    axios.delete(`${apiurl}${db}users/${zip}`).then((res) => {});
  };
...
export const UserForm = ({ db }) => {
  ...
  const onSubmit = (data) => {
    axios
      .post(`${apiurl}${db}users/`, Array(iterations).fill(data))
      .then((res) => {});
  };
